...
class OlmoeForCausalLM(nn.Module, SupportsPP, SupportsLoRA):
    ...
    def __init__(
        ...
        *,
        ...
        zero_delta = torch.zeros(self.config.num_hidden_layers, self.config.hidden_size, self.config.num_experts)
        self.add_steermoe3d_args(zero_delta, n_activated=0, n_deactivated=0, eps=0)

        # * Added

    # * Added

    def add_steermoe3d_args(self, steermoe3d_delta, n_activated, n_deactivated, eps):
        """
        ...
        for layer_idx, layer in enumerate(self.model.layers):
            ...
            layer_moe_block.steermoe3d_delta = steermoe3d_delta[
                layer_idx
            ]  # (experts)
            layer_moe_block.eps = eps
            layer_moe_block.n_activated = n_activated
